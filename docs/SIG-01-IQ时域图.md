...
**轴单位标题**：绘图区左上角竖排「幅度」、右下角单位（`sampleRate` 时显示「时间」，否则「样本」），半透明浮层不占布局；
`axisLabels` prop 控制显隐（默认开启）

**触发显示**：`trigger` prop 启用后进入触发对齐模式（独立于跟随/暂停，持续摄入数据检测触发）：
- `enabled` 开/关；`source:'i'|'q'` 触发通道；`edge:'rising'|'falling'` 边沿；`level` 电平；`mode:'auto'|'normal'|'single'`；`preTrigger` 触发点屏幕位置比例（默认 0.25）
- 检测：新数据段内扫描跨电平边沿（O(段长)，金字塔不动）；触发后显示窗 = `[trigger − pre×span, trigger + (1−pre)×span]`，触发点固定对齐屏幕 pre 处——周期信号稳定重叠，配合余辉形成磷光眼图
- 模式差异：auto 在最近一次 feed 未命中时回退滚动（避免死屏）；normal 无新触发保持定格；single 触发一次即停，`armTrigger()` 重新武装
- 触发窗被环形淘汰整体吞掉时兜底锚定最老可用 span 窗（避免退化为 1 样本窗黑屏）；single 捕获（fired）后整画布冻结保留捕获瞬间（余辉不再衰减），重新武装后恢复
- 交互：电平虚线可拖拽调阈值（Lv 标签随动）；触发点三角标记；滚轮/框选/`+`/`-` 缩放只改窗宽保持对齐；拖拽平移禁用；Alt 标记允许
- 状态角标：⚡ 模式·边沿·电平·状态（single=等待触发/已捕获；auto/normal=触发锁定/等待触发，按最近帧是否命中触发）
- 演示页 trigger 控制组（源/边沿/电平/模式/屏位/arm）

**数据流中断反馈**：跟随态下超过 3 秒未收到实际数据（暂停丢弃的帧不计），顶部居中显示橙色「数据流中断」角标；
恢复跟随/清空时重置计时；演示页「停止推流」按钮可不切页停流验证
