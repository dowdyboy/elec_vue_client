 ...
 * drawAxisOverlay：绘图区底色 → 网格 → 零线 → 边框 → X 刻度短线 → Y 刻度芯片（示波器面板风格）
 * Y 刻度为「内嵌芯片」：半透明底色小圆角片贴在绘图区左缘内侧，
 * 始终画在最上层，密集波形下仍可读，且不占用左侧留白（无动态槽宽抖动）
 * 所有坐标均为 CSS px（调用方需先 ctx.scale(dpr, dpr) 并自行清屏）
 */
...
const CHIP_H = 16 // 芯片高度
const CHIP_R = 4 // 圆角半径
const CHIP_INSET = 6 // 无刻度带时芯片距绘图区左缘 / 有刻度带时距左边框的间距
...
export function drawAxisOverlay(
  ...
  const { plot, xMin, xMax, yMin, yMax, showX = true, showGrid = true, theme } = opts
  ...
  ctx.fillStyle = theme.labelChipBg ?? 'rgba(127,127,127,0.35)'
  for (const t of yTicks) {
    const ty = toPxY(t)
    if (ty < plot.y - 0.5 || ty > plot.y + plot.h + 0.5) continue
    const label = fmtByStep(t, yStep)
    const tw = Math.ceil(ctx.measureText(label).width)
    const cw = tw + CHIP_PAD_X * 2
    let cy = ty - CHIP_H / 2
    // 首尾刻度钳制在绘图区内，避免芯片被裁切
    cy = Math.min(Math.max(cy, plot.y + 2), plot.y + plot.h - CHIP_H - 2)
    // 有左刻度带（plot.x>0）：右对齐贴绘图区左边框；无刻度带：内嵌绘图区左缘
    const cx = plot.x > 0 ? Math.max(2, plot.x - CHIP_INSET - cw) : plot.x + CHIP_INSET
    pathRoundRect(ctx, cx, Math.round(cy) + 0.5, cw, CHIP_H, CHIP_R)
    ctx.fill()
    ctx.fillStyle = theme.text
    ctx.fillText(label, cx + cw / 2, Math.round(cy) + 0.5 + CHIP_H / 2)
    ctx.fillStyle = theme.labelChipBg ?? 'rgba(127,127,127,0.35)'
  }
}
